import axios from 'axios'
import { useEffect, useState } from 'react'
import { endpoint } from '../../index'
import { Link } from 'react-router-dom'
export const Student = () => {
    let token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..DxZmybYVdFPMVmKD_IAli2r5q2_Q26RSruRv5hQoDZg'
    let [ApiData, setApiData] = useState([])
    useEffect(() => {
        async function load() {
            let { data } = await axios.get(endpoint + '/student', { headers: { token } })
            setApiData(data)
        }
        load()
    }, [token])
    return (
        <div className="container" style={{ marginTop: "100px", padding: "0 4%" }}>
            <table className="table mt-5 text-center">
                <thead>
                    <tr>
                        <th >ID</th>
                        <th>UserName</th>
                        <th>Name</th>
                        <th>Phone</th>
                        <th>Address</th>
                        <th>Balance</th>
                        <th>Status</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody>
                    {ApiData && ApiData.map((data, index) => (
                        <tr key={index}>
                            <td>{data._id}</td>
                            <td>{data.UserID.UserName}</td>
                            <td>{data.Name}</td>
                            <td>{data.Phone}</td>
                            <td>{data.Address}</td>
                            <td>{data.Balance}</td>
                            <td>{data.Status}</td>
                            <td>{
                                <div>
                                    <Link to={`/StudentUpdate/${data._id}`} className="btn btn-primary mx-2">Edit</Link>|
                                    <Link to={`/StudentDelete/${data._id}`} className="btn btn-danger mx-2">Delete</Link>
                                </div>
                            }</td>
                        </tr>
                    ))}
                </tbody>
            </table>
        </div>
    )
}